...
var g_jsonviewer = null;

Ext.onReady(function () {

	Ext.BLANK_IMAGE_URL = '/static/bejson/s.gif';	
	Ext.QuickTips.init();
    ...
    var tree = {
        ...
        bbar: [
	        ...
	        new Ext.form.Label({
	        	...
	        	iconCls: Ext.ux.iconCls.get('/static/bejson/arrow_down.png'),
	        	text: 'Next',
	        	handler: function () {
	        		jsonviewer.searchNext();
	        	}
	        }, {
	        	iconCls: Ext.ux.iconCls.get('/static/bejson/arrow_up.png'),
	        	text: 'Previous',
	        	handler: function () {
	...
	var jsonviewer = function () {
		...
		return {
			...
			treebuild: function () {
				...
				root.setIcon(Ext.isArray(json) ? '/static/bejson/array.gif' : '/static/bejson/object.gif');
				this.gridbuild(root);
			},
			...
			json2leaf: function (json) {
				...
				for (var i in json) {
					if (json.hasOwnProperty(i)) {
						if (json[i] === null) {
							ret.push({text: i + ' : null', leaf: true, icon: '/static/bejson/red.gif'});
						} else if (typeof json[i] === 'string') {
							ret.push({text: i + ' : "' + json[i] + '"', leaf: true, icon: '/static/bejson/blue.gif'});
						} else if (typeof json[i] === 'number') {
							ret.push({text: i + ' : ' + json[i], leaf: true, icon: '/static/bejson/green.gif'});
						} else if (typeof json[i] === 'boolean') {
							ret.push({text: i + ' : ' + (json[i] ? 'true' : 'false'), leaf: true, icon: '/static/bejson/yellow.gif'});
						} else if (typeof json[i] === 'object') {
							ret.push({text: i, children: this.json2leaf(json[i]), icon: Ext.isArray(json[i]) ? '/static/bejson/array.gif' : '/static/bejson/object.gif'});
						} else if (typeof json[i] === 'function') {
							ret.push({text: i + ' : function', leaf: true, icon: '/static/bejson/red.gif'});
						}
					}
				}
				return ret;
